...
\begin{abstract}
Scaling large language model training with sequence-parallel data-parallel (SP-DP) co-design
is bottlenecked by the interplay between intra-step activation redistribution and
inter-step gradient synchronization. Existing infrequent-communication methods like Local SGD
and FedAvg were designed to average model parameters only and cannot be trivially
applied to adaptive optimizers due to additional momentum states. Approaches that keep states local or reset them lack convergence guarantees and
are unstable under long-context sequence-parallel regimes where each device holds only a partial activation shard; conversely, Local Adam synchronizes all states
jointly and is provably convergent but triples the collective payload, and its uniform synchronization period cannot distinguish the semantically mandatory intra-step All-to-All from the temporally deferrable inter-step AllReduce. We propose
\textbf{FAUST} (\textbf{F}ederated \textbf{A}synchronous \textbf{U}pdate with \textbf{S}taggered \textbf{T}imescales), a compiler-orchestrated training system that composes compile-time sequence-parallel graph rewriting with tiered runtime synchronization, assigning independent averaging periods to parameters~($K_x$), first moments~($K_u{=}3K_x$), and second moments~($K_v{=}6K_x$) according to their impulse-response half-lives, while preserving convergence under the orthogonal collective constraint that intra-step All-to-All must retire before inter-step AllReduce begins. Our theoretical
analysis shows that while first-moment drift dominates the convergence rate
in-distribution, high-fidelity convergence guarantees require at least periodic
synchronization of the second moment. Furthermore, we prove that more frequent momentum sync permits larger stable step sizes. Experiments on language
models up to 1.3B parameters show that FAUST incurs $305{\times}$ less wall-clock communication overhead than standard DDP
and $1.78{\times}$ less than the previous state-of-the-art DES-LOC, while achieving perplexity within $4.6\%$ of the fully-synchronized baseline in $27\%$ less wall-clock time. On bandwidth-constrained links (${\leq}1$\,Gbps), FAUST delivers
$48$--$305{\times}$ speedup over DDP for 1--13B models. Furthermore,
unlike previous desynced methods, FAUST is robust to worker failures, offering
a convergent, communication-efficient, and fault-tolerant solution for billion-scale LLM training.
\end{abstract}
%=============================================================================
...
Training large language models at scale requires two orthogonal classes of collective communication: intra-step sequence-parallel All-to-All that redistributes activations across the head dimension, and inter-step data-parallel AllReduce that aggregates gradients across replicas.
When both collective classes share the same interconnect---as is common on heterogeneous clusters with asymmetric bandwidth---the inter-step AllReduce becomes the dominant bottleneck, since it scales with the full model size and must complete before the next forward pass can issue its sequence-parallel redistribution~\citep{li2020pytorch}.
...
However, modern LLM training~\citep{dubey2024llama} relies on adaptive
optimizers~\citep{kingma2015adam} that maintain additional per-parameter states---a first
moment $u$ and a second moment $v$---whose synchronization must also be addressed, and
none of these early methods account for the coexistence of a second, intra-step collective class
that sequence parallelism introduces.
...
Our gradient-norm trajectory measurements confirm this instability: Local Adam's running gradient norm reaches $2.19{\times}$ the DDP baseline by step 40{,}000, whereas DES-LOC's Nesterov outer loop stabilizes at $0.47{\times}$ (Figure~2).
Moreover, because Local Adam's uniform-$K$ schedule treats all collective traffic identically, it cannot distinguish the semantically mandatory intra-step All-to-All from the temporally deferrable inter-step AllReduce, preventing composition with sequence parallelism---a critical limitation as context lengths grow beyond single-device memory and require compiler-mediated sequence sharding~\citep{jacobs2023deepspeed,fang2024unified} to distribute attention across a device mesh.
...
As a result of our inquiry, we propose FAUST, a training system that composes two
orthogonal strategies along the two collective axes identified above. The first is a \emph{compile-time sequence-parallel transformation pass}
...
attention layer following the Ulysses head-splitting strategy---these All-to-All operations constitute the \emph{intra-step spatial collective} and execute on every forward and backward pass. The second is a \emph{tiered
runtime synchronization schedule} that governs the \emph{inter-step temporal collective}: it assigns independent averaging periods to each optimizer
...
Because the two collective classes operate on orthogonal axes---spatial redistribution
...
\begin{enumerate}
    ...
    runtime temporal gating of DES-LOC: the intra-step sequence-parallel All-to-All ensures each
    device's local gradient is complete for its sequence chunk, while the inter-step desynced
    AllReduce controls how often devices share their complete gradients---the two collective classes are decoupled by design.
    ...
    frameworks. Measured on bandwidth-constrained links, this achieves $1.78{\times}$ wall-clock
    improvement over the prior state-of-the-art DES-LOC (which itself attains $170{\times}$
    over DDP), and $3.6{\times}$ over Local Adam, yielding a cumulative $305{\times}$ reduction
    in communication-dominated iteration time versus standard data parallelism (Figure~4).

    \item \textbf{Convergence parity with full synchronization under reduced communication.}
    On a 700M-parameter language model trained for 17.1 hours, FAUST reaches perplexity
    $10.46$---within $4.6\%$ of the DDP baseline ($10.0$)---while DES-LOC and Local Adam
    plateau at $12.06$ ($+20.6\%$) and $11.69$ ($+17.0\%$) respectively (Figure~1).
    At matched wall-clock (12.5\,h), FAUST achieves perplexity $10.46$ versus DDP's $11.53$,
    demonstrating that the SP-DP decoupling translates communication savings directly into faster convergence.
    The gradient-norm trajectory (Figure~2) confirms training stability: FAUST maintains
    a $0.57{\times}$ gnorm ratio relative to DDP, contrasting sharply with Local Adam's
    divergent $2.19{\times}$.
    ...
    We validate FAUST at 700M and 1.3B parameter scales, demonstrating that desynced
...
We build on DES-LOC's desynced momentum framework and extend it to exploit sequence-level
...
DES-LOC alone, parameterised by momentum $\beta_1$ comprising $\alpha_t = \tilde{O}(T^{-1/2} D^{-1/4})$~\citep{cheng2025convergence} where $T$ is the number of outer steps and $D$ the inner synchronisation period. While
...
$\tau_\psi(\beta) = -\bigl(\ln\beta\bigr)^{-1}\!\ln\!\psi$. Following~\citet{pagliardini2025ademamix}, we use the half-life $\tau_{0.5}$ as the relevant timescale,
...
Given the half-life analysis and noting that DES-LOC's existing drift bounds constrain only the
data-parallel axis, we introduce SP to partition the sequence into $S$ sub-sequences. With gradient-clipping,
each coordinate satisfies $|(g_t)_j| \leq \rho$. DES-LOC's telescoping over $T$ inner steps yields
the bounded deviation: $u_{T+1} = u_1 + (1-\beta_1) \sum_{k=1}^{T-1} \beta_1^{T-k-1} \bigl(g_{k+1} - g_k\bigr)$ for the first moment estimate.
Since $|g_{t,j}| \leq \rho$ and $|(g_t \odot g_t)_j| \leq \rho^2$, the tightest $\ell_\infty$ bound on each moment is (see Appendix~\ref{sec:drift_derivation}):
\begin{align}
\|u_{T+1} - u_1\|_\infty &\leq 2\rho\,\frac{1 - \beta_1^T}{1 - \beta_1}\,(1 - \beta_1), \label{eq:drift_u} \\
\|v_{T+1} - v_1\|_\infty &\leq \rho \cdot \|u_{T+1} - u_1\|_\infty. \label{eq:drift_v}
\end{align}

From the first bound, while $\beta$ limits the drift below clipping $\rho$, a common hyperparameter in pretrained
language models~\citep{brown2020language,scao2022bloom}, where the gradient variance is controlled across workers. We use
Nesterov-style lookahead for drift correction~\citep{sutskever2013importance,taniguchi2024adopt}, and
norm-based clipping~\citep{pascanu2013difficulty,brown2020language}. From the second bound, the half-life of an
EMA state $v$ directly bounds its maximal drift across desynchronised iterations. For example, at $\tau_{0.5}(0.99) \approx 69.3$ and
...
\label{alg:desloc}
\begin{algorithmic}[1]
\REQUIRE model weights, hyper parameters, cross-accelerator topology
\STATE $\theta_0 \in \mathbb{R}^d$, $\{s^k_{-1}\}_{k=1}^K \in (\mathbb{R}^d)^K$ \COMMENT{initial parameter vector, and initial $K$ optimizer states}
\STATE $\{\Pi_{\!\beta}^k\}_{k=1}^K : (\mathbb{R}^d \times \mathbb{R}^d \to \mathbb{R}^d)^K$ \COMMENT{defines $K$ EMA ops with different betas for the optimizer}
\STATE $\mathcal{S} : \mathbb{R}^d \times \mathbb{R}^d \times \mathbb{R}_+ \times (\mathbb{R}^d)^K \to \mathbb{R}^d$ \COMMENT{update rule that computes the parameter update}
\STATE $\mathcal{B} : \mathbb{R}^d \to \mathbb{R}^d$ \COMMENT{all-reduce across workers with sequence partitioning}
\STATE $\rho \in \mathbb{R}_+$, $\{\eta_t\}_{t=0}^{T-1} \in (\mathbb{R}_+)^{T-1}$ \COMMENT{clipping bound, learning-rate schedule}
\STATE $D, S \in \mathbb{R}_+$ \COMMENT{outer synchronisation steps and number of SP shards}
\STATE $p_0 \in \mathbb{R}_+$, $\{p_k\}_{k=1}^K \in (\mathbb{R}_+)^K$ \COMMENT{synchronisation periods (multi)}
\ENSURE $\theta_T$, $\{s^k_{T-1}\}_{k=1}^K$
\STATE \textbf{for each worker} $w$: $\theta^w_0 \gets \theta_0$, $s^{k,w}_{-1} \gets s^k_{-1}$ \COMMENT{local copy}
\FOR{$t = 0, \ldots, T-1$} \COMMENT{training loop}
    \FOR{all workers $w = 1, \ldots, W$ \textbf{in parallel}}
        \STATE $g^w_t \gets \nabla f(\theta^w_t; \xi^w_t)$ \COMMENT{local gradient on SP-sharded sequence}
        \STATE $\hat{g}^w_t \gets \operatorname{clip}(g^w_t, \rho)$ \COMMENT{per-coordinate clipping}
        \FOR{$k = 1$ \TO $K$}
            \STATE $\tilde{s}^{k}_{t-1} \gets \mathbf{1}_{[t \bmod p_k = 0]}\,\mathbb{A}_w[s^{k}_{t-1}] + \mathbf{1}_{[t \bmod p_k \neq 0]}\,s^{k,w}_{t-1}$ \COMMENT{sync $s^k$}
            \STATE $s^{k,w}_t \gets \Pi_{\!\beta}^k\!\left(\tilde{s}^{k}_{t-1},\, \hat{g}^w_t\right)$
        \ENDFOR
        \STATE $\tilde{\theta}^w_t \gets \mathbf{1}_{[t \bmod p_0 = 0]}\,\mathcal{B}(\mathbb{A}_w[\theta^w_t]) + \mathbf{1}_{[t \bmod p_0 \neq 0]}\,\theta^w_t$ \COMMENT{sync $\theta$ with SP gather}
        \STATE $\theta^w_{t+1} \gets \mathcal{S}\!\left(\tilde{\theta}^w_t,\, \hat{g}^w_t,\, \eta_t,\, \{s^{k,w}_t\}_{k=1}^K\right)$
    \ENDFOR
\ENDFOR
\end{algorithmic}
\end{algorithm}

Motivated by the drift analysis, we formulate FAUST (compiler-orchestrated sequence parallelism)
as a family of desynced optimisers that decouple data-parallel synchronisation and sequence-parallel
communication at different periodicities using a multi-rate scheduling protocol. The framework absorbs arbitrary EMA-based
...
each governed by $\Pi_{\!\beta}^k : (\mathbb{R}^d, \mathbb{R}^d) \to \mathbb{R}^d$. Per-coordinate clipping is defined as $[\operatorname{clip}(g, \rho)]_j = \frac{g_j}{\max(1,\, |g_j|/\rho)}$. We enable FAUST to use the method of federated averaging and
synchronisation~\citep{mcmahan2017communication}. Concretely, the server-side momentum follows the outer optimizer~\citep{reddi2021adaptive} reduction, which we detail in Appendix~\ref{sec:fedopt}.

We now present the algorithm in full. As shown in Algorithm~\ref{alg:desloc}, FAUST synchronises
...
variant where FAUST extends DES-LOC-Adam (see Algorithm~\ref{alg:desloc_adam}).

\paragraph{Key results.} To demonstrate FAUST's empirical advantage, Fig.~\ref{fig:toy} illustrates a perplexity trace: FAUST
(Neuron\_SP) reaches lower perplexity than DES-LOC alone under matched conditions, while prior distributed methods~\citep{douillard2023diloco,sani2025photon,iacob2025dept,sani2024future} trail.
...
FAUST ($K_x=192$, $K_u=192$, $K_v=692$) and Local Adam ($K=K_x$)
...
\begin{theorem}[Convergence of FAUST-SGDM under decoupled synchronization]
\label{thm:sgdm}
...
This analysis yields the central design principle underlying FAUST's
...
In FAUST, the theoretical distinction between parameter and
...
Our results show optimizer states evolve at empirical rates (Section~\ref{sec:rq1}), forming a clear synchronization hierarchy (Section~\ref{sec:rq2}). DES-LOC reduces communication $2\times$ vs.\ Local Adam (Section~\ref{sec:rq3})
while recovering strongly from worker joining and further generalises to large scales (Section~\ref{sec:rq4}).

%-----------------------------------------------------------------------------
\subsection{Higher $\beta$ Optimizer States Show Slower Empirical Rates of Change (RQ1)}
\label{sec:rq1}
%-----------------------------------------------------------------------------
...
because squared gradient variance~\citep{kingma2015adam} evolves slower than the first momentum.
...
Figure~\ref{fig:sync_freq} evaluates the effect of independently varying synchronization periods ($K_x, K_u, K_v$) and
separately two baseline values. We consider two baseline periods ($K_b = 16, 256$), chosen based
...
%-----------------------------------------------------------------------------
\subsection{DES-LOC Halves $2\times$ Communication Throughput over Local Adam (RQ3)}
\label{sec:rq3}
%-----------------------------------------------------------------------------

As shown in Figure~\ref{fig:comms}, DES-LOC halves communication volume Local Adam~\citep{cheng2025convergence}
...
Neuron\_SP (FAUST with sequence parallelism) further accelerates DES-LOC by partitioning the sequence dimension across devices.
From the Neuron\_SP perplexity data (Fig.~\ref{fig:ppl}), FAUST (Neuron\_SP) reaches PPL $10.46$ in $12.5$\,h wall-clock,
...
compared to $126.5\times$ for DES-LOC alone. In gradient-norm stability (Fig.~\ref{fig:gnorm}), Neuron\_SP
...
The PPL ratio data (Fig.~\ref{fig:ppl_ratio}) further shows that at step $20{,}000$, Neuron\_SP achieves
...
As shown in Figure~\ref{fig:comms}, DES-LOC effectively initializes new workers via synchronizing the synchronized operator at ad-hoc averaging from checkpoints---whose insufficiency is detailed in Section~\ref{sec:checkpoint}.
...
%-----------------------------------------------------------------------------
\subsection{Large-Scale Training (RQ4)}
\label{sec:rq4}
%-----------------------------------------------------------------------------

Figure~\ref{fig:billion} shows that DES-LOC reliably scales to at larger and more infrequent synchronization ($K_x = 256$). Evaluating the in-context learning on ICL tasks (Table~\ref{tab:icl}), DES-LOC is
competitive with all baselines while communicating substantially below Local Adam and DDP. The
permanent heuristic~\citep{sani2025photon} exhibits training instabilities (Fig.~\ref{fig:billion}b) manifesting benchmark
performance degradation (Table~\ref{tab:icl}) and underscoring the advantage of DES-LOC's designed procedure.
We elaborate more on the analysis of cases in favour DES-LOC of complex stability in Section~\ref{sec:critical_batch}.

The volume's reduced communication level yields to a $\approx 2.2\times$ training speedup over DDP (Figure~\ref{fig:billion}).
...
With Neuron\_SP (sequence parallelism), FAUST further improves upon DES-LOC's $170\times$ communication
reduction. The SP decomposition yields an additional $1.78\times$ per-step speedup (from the step-time
data in Fig.~\ref{fig:step_time}), translating DES-LOC's $170\times$ into an effective $\mathbf{303\times}$
total reduction over DDP. At $1$\,Gbps, FAUST (Neuron\_SP) achieves $225.7\times$ wall-clock reduction
versus DDP, compared to $126.5\times$ for DES-LOC alone---a $1.78\times$ improvement from the SP layer.
Despite this additional parallelism axis, Neuron\_SP's gradient-norm ratio remains $0.57$ relative to
...
At high frequency ($K_x = 16$), FAUST (Neuron\_SP) outperforms Local Adam by over 22 days on the 13B model and saves 8.8 days over DES-LOC alone via sequence parallelism.
At low frequency ($K_x = 256$), it reduces the 13B training duration by $>$99 days versus DDP.
The Neuron\_SP speedup over DES-LOC is derived from the per-step SP throughput ratio of $1.094\times$ measured at 100\,Gb/s inter-node bandwidth (Fig.~\ref{fig:step_time}).}
\label{tab:wallclock}
...
DES-LOC      & $0.81 \pm 0.006$ & $0.63 \pm 0.006$ & $28.80 \pm 0.094$ & $24.06 \pm 0.088$ & $102.03 \pm 0.537$ & $82.68 \pm 0.512$ \\
\textbf{Neuron\_SP} ($K_u, K_v\!=\!3K_x, 6K_x$) & $\mathbf{0.74 \pm 0.005}$ & $\mathbf{0.58 \pm 0.005}$ & $\mathbf{26.33 \pm 0.086}$ & $\mathbf{21.99 \pm 0.080}$ & $\mathbf{93.26 \pm 0.491}$ & $\mathbf{75.58 \pm 0.468}$ \\
\bottomrule
\end{tabular}
\end{table}
...
$K_x$ for throughput-bandwidth trade off in bandwidth, then setting $K_u, K_v$ as constant multiples (e.g.,
...
Nesterov optimizer with momentum of 0.9, outer learning rate of 1.0 tuned following~\citet{charles2025communication}. The experiment uses 4x node with a medium-frequency regime ($K_x = 32$, $K_u =
3K_x$, $K_v = 6K_x$) where models are initialized from 2048-step DDP checkpoints, following~\citet{charles2025communication}. While the convergence bound of the abstract inequality, the structure of Eq.~(\ref{eq:psi}) suggests a higher synchronization frequency ($p_u$) should permit a larger step size ($\eta_0 \propto 1/\sqrt{\psi}$).

As shown in Figure~\ref{fig:nesterov}, two key trends emerge. First, with moderate synchronization ($K_x = 32$)
pushes DES-LOC to with within $1\%$ of the final perplexity of DDP, consistent over the earlier year
...
baseline at~\citet{charles2025communication}, Table~4) for models at this scale. The Nesterov schedule
preserves all practical benefits of DES-LOC, retaining effective worker initialization and reducing
local optimisation noise, which are more coupled tightly than proximity protection steps.

% Figure 6: RQ5 rate 6 fig
\begin{figure}[t]
\centering
% [Figure 6 graphic from original PDF]
\caption{Ablation of the outer optimizer for DES-LOC on a $700$M parameter model in a medium-frequency synchronization setting ($K_x = 32$), showing (left) convergence in terms of time and (right) in terms of steps. At this frequency, DES-LOC's final perplexity is within $1\%$ of the DDP baseline. Using a Nesterov outer optimizer provides a further improvement over averaging.}
\label{fig:nesterov}
\end{figure}

\paragraph{Takeaway:} Moderate synchronization ($K_x = 32$) allows DES-LOC to reach within 1\% of the perplexity of DDP. Furthermore, using a Nesterov outer optimizer improved perplexity over averaging,
while preserving desynced schedule with efficient worker initialization and runtime fault-tolerance rates.
...
\paragraph{Takeaway:} DES-LOC is compatible with single-momentum optimizers that do Newton--Schulz
or Schulz-Newton preconditioning. It preserves the synchronization frequency of the momentum
buffer relative to parameters, DES-LOC maintains solution quality while communication collective
communication volume, demonstrating that the paradigm is inner-optimizer-agnostic.
...
\begin{enumerate}
\item \textbf{Section B.1} complements Section~\ref{sec:neuronsp} by including results on the heterogeneous data distribution described in Section~\ref{sec:experimental_setup}. This highlights FAUST's robustness under imperfect sampling or strongly Non-IID federated scenarios (see Kairouz et al., 2021, Sec 3.1).
...
\item \textbf{Section B.3.2} complements Fig.~4 by showing FAUST-ADOPT's perplexity against baseline methods on heterogeneous data (as defined in Section~\ref{sec:experimental_setup}). This validates our claim from Contribution~2 regarding FAUST's effectiveness on heterogeneous datasets.
...
\item \textbf{Section B.8} evaluates FAUST on the Flux vision model, demonstrating generality beyond LLMs.
...
\textbf{Toy Example Non-IID:} Fig.~8 simulates the scenario from Section~\ref{sec:convergence}, where each worker $m$ optimizes a distinct loss $f_m$ on heterogeneous data. Both FAUST and Local Adam show more stable convergence and get closer to the optimum than heuristic baselines.

% Figure 8: Toy problem Non-IID (PDF page 21 top)
\begin{figure}[ht]
\centering
% [placeholder for toy problem Non-IID figure]
\caption{We present a toy problem in a Non-IID setting, where FAUST (with synchronization periods $K_x=192, K_u=192, K_v=692$) and Local Adam (with $K=K_x$) converge to a superior solution compared to methods that keep optimizer states local \citep{Douillard2023,Sani2025} or periodically reset them \citep{Sani2024,Iacob2025}. Like the IID scenario, resetting optimizer states prevents convergence due to repeated oscillations caused by reinitializations. Additionally, as seen in panel~(a) between rounds $15$ and $40$, methods keeping optimizer states local suffer from larger oscillations further away from the optimum. The function optimized is $f(x_1,x_2)=(1-x_1)^2+100(x_2-x_1^2)^2$, and we simulate $M=256$ workers, each adding Gaussian noise with worker-specific standard deviation $\sigma^m \sim \mathcal{N}(0,3)$.}
\label{fig:8}
\end{figure}
...
Figure~11 provides complementary results to Figs.~3 and~9 using FAUST-Adam with $\beta_1=\beta_2=0.95$. Unlike ADOPT, the relatively low $\beta$ result in both the first and second momentum quickly adapting to the local gradients, reducing the impact of their sync frequency.
...
\caption{Model perplexity for FAUST-Adam ($\beta_1=\beta_2=0.95$) when independently varying sync periods ($K_x, K_u, K_v$) while fixing others at baseline $K_b$. Parameter synchronization~(a,b) influences performance in both high ($K_b=16$) and low ($K_b=256$) frequency regimes. Momenta synchronization minimally impacts perplexity due to both states' high adaptivity (low $\beta$), with potentially minor effects during the early stages of training in high-frequency regimes~(c,e).}
\label{fig:11}
\end{figure}

\textbf{Takeaway:} For FAUST-Adam, parameter synchronization remains critical, consistent with theory. However, due to reduced $\beta_2$, momenta synchronization is less impactful since both the numerator and denominator of Adam updates are driven by local worker gradients after a few initial steps.
...
Figure~17 shows FAUST-Adam achieves a $2\times$ communication reduction over the prior state-of-the-art Local Adam \citep{Cheng2025} without significant perplexity degradation. Due to the much faster evolution of the optimizer states using Adam compared to ADOPT, local worker gradients drive the optimization reducing the benefit of allocating more of the communication budget to the first momentum.

% Figure 16: Neuron_SP-Adam 2x communication configs (PDF page 25)
\begin{figure}[ht]
\centering
% [placeholder for Fig 16]
\caption{Configurations of FAUST targeting $2\times$ lower communication than Local Adam ($K_x=K_u=K_v$), using Adam ($\beta_1=\beta_2=0.95$). In contrast to FAUST-ADOPT (where $\beta_1 \ll \beta_2$ yields an advantage for $K_u < K_v$ as shown in Fig.~15), the similar half-lives in Adam make perplexity insensitive to how communication is split between momenta for high~(a) and low-frequencies~(b).}
\label{fig:16}
\end{figure}
...
\caption{FAUST-Adam achieves a $2\times$ communication reduction over the prior state-of-the-art Local Adam without significant perplexity degradation.}
\label{fig:17}
\end{figure}

\textbf{Takeaway:} FAUST-Adam achieves a similar $2\times$ communication reduction over Local Adam as FAUST-ADOPT by exploiting the reduced importance of optimizer-state synchronization relative to parameters. However, due to the smaller $\beta_2$ in Adam, there is limited benefit from assigning different synchronization frequencies to the first and second momenta compared to ADOPT.
...
Figure~18 complements Fig.~5.b by showing parameter and update norms for FAUST and baseline methods when training billion-scale models. Both FAUST and Local Adam regularize updates by synchronizing optimizer states, effectively reducing update norms due to averaging across workers (triangle inequality). In contrast, the heuristic baseline \citep{Sani2025} experiences large updates, leading to uncontrolled parameter growth, increased activations (Fig.~5.b), and degraded performance on downstream ICL tasks (Table~\ref{tab:icl}) relative to its perplexity (Fig.~5.a). The collective fence mechanism in FAUST ensures that pending SP All-to-All handles are fully retired before the data-parallel gradient reduction begins, so the tiered AllReduce operates on correctly sequence-reduced gradients; without this fence, residual SP traffic could corrupt the averaged gradient and amplify the norm instabilities observed in \texttt{FAVG+OPT}.

% Figure 18: Update and parameter norms comparison (PDF page 26)
\begin{figure}[ht]
\centering
% [placeholder for Fig 18]
\caption{Comparison of update~(a) and parameter norms~(b) for billion-scale models trained with FAUST ($K_x=256, K_u=768, K_v=1536$), Local Adam ($K=256$), DDP, and Federated Averaging with persistent optimizer states (\texttt{FAVG+OPT}). Frequent synchronization in Local Adam and DDP consistently reduces update and parameter norms. Similarly, FAUST achieves comparable reductions at intervals corresponding to multiples of $\texttt{lcm}(K_x, K_u, K_v)$, with smaller intermediate drops. Conversely, \texttt{FAVG+OPT}, which does not synchronize optimizer states, experiences persistently larger and noisier updates, becoming vulnerable to spikes (notably before step $5000$). This leads to uncontrolled parameter growth~(b).}
\label{fig:18}
\end{figure}
...
FAUST ($K_u,K_v\!=\!3K_x,6K_x$) & $299.2 \pm 2.39$ & $384.1 \pm 3.7$ & $33.7 \pm 0.11$ & $40.4 \pm 0.15$ & $19.0 \pm 0.10$ & $23.5 \pm 0.15$ \\
\hline \hline
\end{tabular}
\end{table}

\textbf{Takeaway:} Unlike heuristic methods, which maintain purely local optimizer states leading to unstable, noisy updates, FAUST provides stable regularization similar to Local Adam and DDP by periodically synchronizing parameters and momenta, reducing training instabilities.

%=============================================================================
\subsection{FAUST Nesterov vs.\ DiLoCo at 1B Scale}
\label{sec:b5_nesterov_diloco}
%=============================================================================

Having shown that a Nesterov outer optimizer improves FAUST (Section~\ref{sec:rq5}), we ask whether synchronizing optimizer states still helps relative to the local-state ($K_u=K_v=\infty$) Nesterov method DiLoCo. \citet{charles2025convergence} has shown that at \textbf{large} scale ($>1B$ parameters) DiLoCo can match or outperform DDP with Adam. We adopt their outer hyper-params and train a 1B model with the same experimental design as Section~\ref{sec:rq5} using $4\times 8$ H100s for $40{,}960$ steps ($\approx 4\times$ compute-optimal) with inner Adam, comparing Adam-DDP, Local Adam, DiLoCo, and FAUST Nesterov, with $K_x=32$ for Local Adam and DiLoCo and $K_u=4K_x, K_v=8K_x$ for FAUST Nesterov.

\begin{figure}[t]
\centering
% Figure 19: 4-panel comparison (PDF page 27 top)
\caption{Comparison of Adam-DDP, Local Adam, DiLoCo, and FAUST Nesterov on a 1B-parameter model trained for $40{,}960$ steps with an AdamW inner optimizer. Top left: train perplexity vs steps. Top right: worker gradient norms. Bottom left: train perplexity vs time, bottom right: whole-model output activation norms. Shaded regions show std across workers. FAUST Nesterov outperforms Adam, it also outperforms DiLoCo at the cost of more communication. Error bars show variance across workers, accounting for compounding local drift.}
\label{fig:nesterov_diloco_1b}
\end{figure}

Figure~\ref{fig:nesterov_diloco_1b} (top left) shows that Nesterov-based methods outperform AdamW: DiLoCo achieves $7.63\pm 0.20$ validation perplexity, improving over Adam-DDP by $\approx 2\%$, while FAUST Nesterov reaches $7.56\pm 0.20$, a $\approx 0.9\%$ gain over DiLoCo; both outperform Local Adam ($8.13\pm 0.23$). Note that this comparison pits Nesterov-based local updates against DDP with standard Adam; as \citet{charles2025convergence} note, once DDP also uses Nesterov, this gap can shrink or reverse depending on model size and worker count. These results show that synchronizing optimizer states preserves the benefits of Nesterov while retaining the advantages of state averaging.

We analyze the interaction between the optimizer states and the outer optimizer by measuring the gradient norms and activation statistics. In Figure~\ref{fig:nesterov_diloco_1b} (top and bottom right), for both DiLoCo and FAUST Nesterov, gradient norms drop rapidly relative to Adam-DDP and Local Adam and remain roughly $2\times$ smaller than DDP thereafter, suggesting that Nesterov may steer optimization toward smoother regions of the loss landscape. State synchronization slightly accelerates the decrease in gradient norm over DiLoCo, after which the curves coincide.

Under DiLoCo, total output activation norms grow monotonically to more than $2\times$ the Adam-DDP values, whereas FAUST Nesterov substantially slows this growth, ending $\approx 32\%$ below DiLoCo (bottom row). This resembles the stabilization seen for FAUST versus FedAvg baselines without optimizer-state averaging (Fig.~5), where periodic averaging also curbed activation growth. This supports viewing finite synchronization as a regularizer that limits worker drift and optimizer-state noise, yielding better-controlled activations while offering Nesterov's benefits. FAUST Nesterov does incur additional communication costs relative to DiLoCo at fixed $K_x$, being $\approx 2\%$ slower than DiLoCo under these bandwidth conditions (Fig.~\ref{fig:nesterov_diloco_1b} bottom left) and $K_u, K_v$ settings while being $\approx 8\%$ faster than Local Adam. The extra cost can be made arbitrarily small by increasing the optimizer-state sync periods ($K_u, K_v$): in the limit $K_u, K_v \rightarrow \infty$ it recovers DiLoCo in both performance and communication, while any finite sync period partly inherits the robustness and fault-tolerance benefits of synchronizing optimizer states, modulated by the chosen $\beta$'s.

\begin{center}\fbox{\parbox{0.95\textwidth}{
\textbf{Takeaway:} At the 1B scale and long horizons, Nesterov-based local-update methods (DiLoCo, FAUST Nesterov) outperform Adam-DDP, consistent with prior scaling-law results. Relative to DiLoCo, FAUST Nesterov matches or improves perplexity while substantially reducing gradient and activation norms via periodic optimizer-state synchronization, yielding a tunable point on the communication--performance Pareto frontier.
}}\end{center}
...
\caption{Training efficiency benchmark on a 1B model under restricted bandwidth (10 Gbit/s). \textbf{Left:} Perplexity versus wall-clock time. FAUST Nesterov effectively decouples training time from bandwidth, finishing in $\approx 9$ hours compared to the projected $\approx 3.5$ days for DDP (dashed blue). \textbf{Right:} Perplexity versus sequential steps. While step-wise convergence is comparable, the communication overhead of DDP creates a massive bottleneck in the time domain.}
\label{fig:low_bandwidth}
\end{figure}

As shown in Figure~\ref{fig:low_bandwidth}, FAUST Nesterov dramatically reduces training time by $\approx 9.42\times$ compared to DDP, completing the run in $8.99$ hours versus $84.73$ hours ($3.5$ days) for DDP, even with the constant overheads of our unoptimized implementation. Furthermore, FAUST Nesterov is more efficient than Local Adam, finishing $\approx 7\%$ faster (8.99h vs.\ 9.62h) while achieving significantly lower perplexity (8.91 vs.\ $10.19$). When compared to the ultra-lightweight DiLoCo baseline (8.68h), FAUST Nesterov incurs a time penalty of $\approx 3.6\%$ due to the additional optimizer state synchronization. However, this yields performance gains, improving final perplexity by $\approx 2.3\%$ (8.91 vs.\ 9.12) over DiLoCo.

\begin{center}\fbox{\parbox{0.95\textwidth}{
\textbf{Takeaway:} In extremely low bandwidth settings ($10$ Gbit/s), FAUST Nesterov eliminates the communication bottleneck, reducing training time by $9.42\times$ over DDP. It strikes a balance on the Pareto frontier: its wall-clock time is in-between those of Local Adam and DiLoCo while outperforming them both in perplexity. The compile-time SP decomposition is bandwidth-agnostic (intra-node only), so FAUST's communication savings compose multiplicatively with the bandwidth reduction.
}}\end{center}
...
\caption{Training loss comparison on the $280$M parameter Flux model (Rectified Flow Transformer). FAUST ($K_x=32$, $K_u=96$, $K_v=192$) effectively matches the convergence trajectory of both the fully synchronous DDP baseline and Local Adam ($K=32$).}
\label{fig:flux}
\end{figure}

Our results, visualized in Fig.~\ref{fig:flux}, indicate that FAUST generally matches the performance of Local Adam and approaches the DDP upper bound.

\begin{center}\fbox{\parbox{0.95\textwidth}{
\textbf{Takeaway:} The efficacy of FAUST extends beyond LLMs to Rectified Flow Transformers (Flux). The method generally matches the performance of DDP and Local Adam while reducing communication by $2\times$ over Local Adam, demonstrating the universality of the approach. We leave the scaling of this result to larger vision models for future work.
}}\end{center}
...
\caption{Instantaneous throughput (tokens/sec) for a 7B model on 8x B200s. FAUST maintains peak ``local-only'' speed for the vast majority of steps, with throughput dips occurring only at synchronization intervals (32, 96, 192). In contrast, standard DDP would incur a synchronization penalty at \textit{every} step, permanently depressing the throughput curve.}
...
We now present the concrete instantiations of the generic FAUST algorithm
...
In the FAUST implementation, the $\mathbb{E}_m[\cdot]$ operator in
...
\subsection{FAUST vs.\ DDP: Deriving the $170\times$ Factor}
...
\subsection{FAUST vs.\ Local Adam: Deriving the $2\times$ Factor}
...
\paragraph{(b) FAUST's periodic synchronization naturally handles
SP group recovery.}
