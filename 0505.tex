...
\rowcolor{gray!5}
& Local Adam     & 52.8 & 23.7 & 40.3 & 70.1 & 53.1 & 48.0 \\
& FAVG+OPT       & 50.1 & 22.3 & 38.6 & 68.9 & 51.8 & 46.3 \\
\rowcolor{gray!5}
& DES-LOC        & 52.6 & 23.9 & 40.1 & 70.0 & 53.0 & 47.9 \\
\rowcolor[HTML]{F6FCF8}
& \textbf{FAUST} & \textbf{52.9} & \textbf{24.0} & \textbf{40.5} & \textbf{70.3} & \textbf{53.2} & \textbf{48.2} \\
\midrule[1.1pt]
\multirow{4}{*}{\rotatebox{90}{\small Ablation}}
& \quad $K_u{=}K_x,\;K_v{=}K_x$   & 52.4 & 23.5 & 39.9 & 69.8 & 52.7 & 47.7 \\
\rowcolor{gray!5}
& \quad $K_u{=}6K_x,\;K_v{=}3K_x$ & 51.7 & 23.1 & 39.4 & 69.5 & 52.3 & 47.2 \\
& \quad $P_{\text{SP}}{=}1$ (no parallel) & 52.5 & 23.8 & 40.0 & 69.9 & 52.9 & 47.8 \\
\rowcolor{gray!5}
& \quad $P_{\text{SP}}{=}4$        & 52.7 & 23.9 & 40.3 & 70.1 & 53.0 & 48.0 \\
\bottomrule[1.6pt]
\end{tabular}%
}
\end{table}
...
\paragraph{Compile-time SP ablation.}
...
\paragraph{Communication-efficient parallel training.}
Synchronous data parallelism exchanges gradients every step via Ring-AllReduce~\citep{sergeev2018horovodfasteasydistributed,li2020pytorch}, incurring communication linear in model size that dominates wall-clock time on weakly connected hardware~\citep{sani2024photonfederatedllmpretraining}. FedAvg~\citep{mcmahan2017communication} and Local SGD~\citep{stich2019local} amortize this over $K$ local steps, with SCAFFOLD~\citep{karimireddy2021scaffoldstochasticcontrolledaveraging} correcting client drift. Orthogonally, sequence parallelism partitions the sequence dimension: DeepSpeed Ulysses~\citep{jacobs2023deepspeedulyssesoptimizationsenabling} inserts head-splitting All-to-All collectives around attention, and USP~\citep{fang2024uspunifiedsequenceparallelism} unifies ring and Ulysses strategies. 

\paragraph{Adaptive optimizers and their distributed variants.}
Adam~\citep{kingma2017adammethodstochasticoptimization} dominates LLM pre-training~\citep{grattafiori2024llama3herdmodels,brown2020languagemodelsfewshotlearners}, scaling to large batches where SGD stalls~\citep{kunstner2023noisemainfactorgap}; however, convergence requires $\beta_1 < \sqrt{\beta_2}$~\citep{reddi2019convergenceadam,zhang2023adamconvergemodificationupdate}, relaxed by ADOPT~\citep{taniguchi2024adoptmodifiedadamconverge} and AMSGrad~\citep{chen2019convergenceclassadamtypealgorithms}. Momentum-based~\citep{pmlr-v28-sutskever13,you2020largebatchoptimizationdeep} and learned~\citep{chen2023symbolicdiscoveryoptimizationalgorithms} optimizers also track gradient moments. Distributing Adam is harder than SGD: ad-hoc extensions keep states local~\citep{douillard2024dilocodistributedlowcommunicationtraining,hilmkil2021scalingfederatedlearningfinetuning,liu2024asynchronouslocalsgdtraininglanguage} or reset them after sync~\citep{sani2024futurelargelanguagemodel,sani2024photonfederatedllmpretraining}, both lacking convergence guarantees. Local Adam~\citep{cheng2025convergencedistributedadaptiveoptimization} provides the first provable analysis but triples communication by syncing all states jointly. FAUST decouples these syncs via half-life driven tiering, halving Local Adam's communication while preserving convergence.
