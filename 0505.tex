...
$\rho_{\text{eff}} = \rho\,/\sqrt{P}$ into~\eqref{eq:drift_u}--\eqref{eq:drift_v} for the
...
(DP state averaging). We formulate this as:
\begin{equation}
...
All-to-All operators inserted by Algorithm~\ref{alg:compile} (where $L$
...
composition from Algorithm~\ref{alg:compile}, and let
...
by $f^*$. For each tier $k$ and replica $m$, the local objective satisfies
a \emph{tier-specific descent lemma}: for all $\theta$,
\begin{equation}
...
These assumptions subsume the standard regularity
...
is the SP precision ratio (negligible under FP32 accumulation).

Then the FAUST potential contracts as
\begin{equation}
\mathcal{V}_T
\;\leq\;
...
In FAUST, the tiered gradient-bucket structure
(Section~\ref{sec:desloc_algorithm}) operationalises the per-tier
...
(Eq.~\eqref{eq:sp_grad_equiv}). The collective fence
(Algorithm~\ref{alg:desloc}, line~13) ensures all SP handles retire before
...
SmolLM2~\citep{allal2025smollm2}: 70\% Fineweb-Edu~\citep{sardana2025chinpenedo2024finewebdatasetsdecantingwebchillaoptimalaccountinginferencelanguage},
...
($2\times$ compute-optimal)~\citep{sardana2025chinchillaoptimalaccountinginferencelanguage}. Non-IID data
...
%-----------------------------------------------------------------------------
\subsection{Parameters Require Frequent Sync, Momenta Sync Proportional to \texorpdfstring{$\beta$}{beta} (RQ2)}
\label{sec:rq2}
...
%-----------------------------------------------------------------------------
\subsection{DES-LOC Halves \texorpdfstring{$2\times$}{2x} Communication Throughput over Local Adam (RQ3)}
\label{sec:rq3}
%-----------------------------------------------------------------------------

\begin{figure}[ht]
\centering
\includegraphics[width=0.8\textwidth]{figure1_ppl_ratio.png}
\caption{Perplexity ratio of each method relative to the DDP baseline across training steps.
FAUST  achieves the lowest PPL ratio, confirming that the SP-DP decoupling translates
communication savings directly into faster convergence.}
\label{fig:ppl_ratio}
\label{fig:ppl}
\end{figure}
...
\begin{figure}[ht]
\centering
\includegraphics[width=0.8\textwidth]{figure2_gnorm_ratio.png}
...
by step $40{,}000$. DES-LOC's Nesterov outer loop stabilizes at $0.47{\times}$.}
\label{fig:gnorm}
\end{figure}
...
From the perplexity data (Fig.~\ref{fig:ppl}), FAUST  reaches PPL $10.46$ in $12.5$\,h wall-clock,
...
At high frequency ($K_x = 16$), FAUST outperforms Local Adam by over 22 days on the 13B model and saves 8.8 days over DES-LOC alone via sequence parallelism.
At low frequency ($K_x = 256$), it reduces the 13B training duration by $>$99 days versus DDP.
The FAUST speedup over DES-LOC is derived from the per-step SP throughput ratio of $1.094\times$ measured at 100\,Gb/s inter-node bandwidth (Fig.~\ref{fig:step_time}).}
\label{tab:wallclock}

\resizebox{\linewidth}{!}{%
\begin{tabular}{l|cc|cc|cc}
\toprule
& \multicolumn{2}{c|}{1B Model}
& \multicolumn{2}{c|}{7B Model}
...
\begin{table}[ht]
\centering 
\caption{Zero-shot in-context learning (ICL) benchmark accuracy and ablation study for the 1.3B model. \textbf{Top}: comparison of FAUST against baselines ($K_x = 256$, $K_u = 3K_x$, $K_v = 6K_x$). DES-LOC is competitive with DDP and Local Adam across all benchmarks, while FAVG+OPT degrades on several tasks due to training instabilities. \textbf{Bottom}: ablation on synchronization schedule and compile-time SP degree. Best communication-efficient method in \textbf{bold}.} \label{tab:icl} 
\resizebox{\linewidth}{!}{%
\begin{tabular}{llcccccc} \toprule & \textbf{Method} & \textbf{ARC-E} & \textbf{ARC-C} & \textbf{HellaSwag} & \textbf{PIQA} & \textbf{WinoGrande} & \textbf{Avg.} \\ \midrule \multirow{5}{*}{\rotatebox{90}{\small Baselines}} & DDP (baseline) & 53.2 & 24.1 & 40.8 & 70.5 & 53.4 & 48.4 \\ & Local Adam & 52.8 & 23.7 & 40.3 & 70.1 & 53.1 & 48.0 \\ & FAVG+OPT & 50.1 & 22.3 & 38.6 & 68.9 & 51.8 & 46.3 \\ & DES-LOC & 52.6 & 23.9 & 40.1 & 70.0 & 53.0 & 47.9 \\ & \textbf{FAUST } & \textbf{52.9} & \textbf{24.0} & \textbf{40.5} & \textbf{70.3} & \textbf{53.2} & \textbf{48.2} \\
\midrule
\multirow{4}{*}{\rotatebox{90}{\small Ablation}} & \quad $K_u{=}K_x,\; K_v{=}K_x$ (uniform) & 52.4 & 23.5 & 39.9 & 69.8 & 52.7 & 47.7 \\ & \quad $K_u{=}6K_x,\; K_v{=}3K_x$ (inverted) & 51.7 & 23.1 & 39.4 & 69.5 & 52.3 & 47.2 \\ & \quad $P_{\text{SP}}{=}1$ (no paraller) & 52.5 & 23.8 & 40.0 & 69.9 & 52.9 & 47.8 \\ & \quad $P_{\text{SP}}{=}4$ & 52.7 & 23.9 & 40.3 & 70.1 & 53.0 & 48.0 \\ \bottomrule \end{tabular}
}
\end{table}
...
\paragraph{Compile-time SP ablation: AutoSP vs.\ baselines.}
...
AutoSP---the graph-rewriting pass that inserts head-dimension All-to-All collectives around
...
RingAttention: at $8$K sequence length on the 8B model, AutoSP takes $9$\,s versus $10$\,s (ZeRO-3)
and $9$\,s (RingAttention/DS-Ulysses), while at $24$K it completes in $31$\,s where ZeRO-3 encounters OOM.
This confirms that the compile-time SP decomposition increases the memory-bounded trainability frontier
without imposing a per-step latency penalty, validating the design
assumption that intra-step All-to-All can be decoupled from inter-step AllReduce
without contention.

\begin{table}[ht]
\centering
\caption{Compile-time SP ablation on Llama-3.1 (H100 GPUs). \textbf{Left columns}: maximum
sequence length before OOM across model scales. \textbf{Right columns}: per-iteration time (seconds,
averaged over 10 iterations) for the 8B model at increasing sequence lengths.
OOM denotes out-of-memory. AutoSP extends trainable context by $2.14$--$5{\times}$
over ZeRO-3 while maintaining comparable iteration time.}
\label{tab:sp_ablation}
\begin{tabular}{l|ccc|ccc}
\toprule
& \multicolumn{3}{c|}{\textbf{Max Seq.\ Length (K)}} & \multicolumn{3}{c}{\textbf{Iter.\ Time (s) @ 8B}} \\
\textbf{Method} & 3B & 8B & 13B & 8K & 24K & 90K \\
\midrule
ZeRO-3        & 15K & 8K  & 6K  & 10 & OOM & OOM \\
RingAttention  & 35K & 15K & 9K  & 46 & 29  & OOM \\
DS-Ulysses     & 35K & 15K & 8K  & 9  & 26  & OOM \\
\textbf{AutoSP (ours)} & \textbf{75K} & \textbf{45K} & \textbf{15K} & 9 & 31 & 199 \\
\bottomrule
\end{tabular}
\end{table}
...
%=============================================================================
In this paper, we present AutoSP, the first compiler-based, PyTorch-native solution for training
large-language-models at long-contexts. Through a combination of automated sequence-parallelism
(SP) and a sequence-aware AC strategy, AutoSP achieves significant sequence length extensions at
negligible cost to training throughput. Our results demonstrate that compiler-driven, PyTorch-native
automation provides a practical and portable foundation for long-context model training.
...
\caption{We present a toy problem in a Non-IID setting, where FAUST (with synchronization periods $K_x=192, K_u=192, K_v=692$) and Local Adam (with $K=K_x$) converge to a superior solution compared to methods that keep optimizer states local \citep{douillard2024dilocodistributedlowcommunicationtraining,sani2024photonfederatedllmpretraining} or periodically reset them \citep{sani2024futurelargelanguagemodel,zhang2024deptdecoupledprompttuning}. Like the IID scenario, resetting optimizer states prevents convergence due to repeated oscillations caused by reinitializations. Additionally, as seen in panel~(a) between rounds $15$ and $40$, methods keeping optimizer states local suffer from larger oscillations further away from the optimum. The function optimized is $f(x_1,x_2)=(1-x_1)^2+100(x_2-x_1^2)^2$, and we simulate $M=256$ workers, each adding Gaussian noise with worker-specific standard deviation $\sigma^m \sim \mathcal{N}(0,3)$.}
\label{fig:8}
\label{fig:noniid}
\end{figure}
...
Figure~17 shows FAUST-Adam achieves a $2\times$ communication reduction over the prior state-of-the-art Local Adam \citep{cheng2025convergencedistributedadaptiveoptimization} without significant perplexity degradation. Due to the much faster evolution of the optimizer states using Adam compared to ADOPT, local worker gradients drive the optimization reducing the benefit of allocating more of the communication budget to the first momentum.
...
\begin{tabular}{l|cc|cc|cc}
\toprule
& \multicolumn{2}{c|}{\textbf{1B Model}}
& \multicolumn{2}{c|}{\textbf{7B Model}}
& \multicolumn{2}{c}{\textbf{13B Model}} \\
\textbf{Method}
& $K_x{=}16$ & $K_x{=}256$
& $K_x{=}16$ & $K_x{=}256$
& $K_x{=}16$ & $K_x{=}256$ \\
...
FAUST ($K_u,\ K_v{=}3K_x,\ 6K_x$)
& $299.2^{\pm 2.39}$ & $384.1^{\pm 3.7}$
& $33.7^{\pm 0.11}$ & $40.4^{\pm 0.15}$
& $19.0^{\pm 0.10}$ & $23.5^{\pm 0.15}$ \\
\bottomrule
\end{tabular}%
}
\end{table}
...
\STATE $\{\eta_t\}_{t=0}^{T-1} \subset \mathbb{R}_{>0}$ \COMMENT{$K_x$-aligned WSD step-size schedule}
...
\STATE $K_x, K_u, K_v \in \mathbb{N}_+$ \COMMENT{sync periods for parameters, first moments, and second moments}
\ENSURE $x_T, u_{T-1}, v_{T-1}$

\STATE For each worker $m$: initialize $x^m_0 = x_0$ and $u^m_{-1} = v^m_{-1} = 0$.

\FOR{$t = 0, \ldots, T-1$}
    \FORALL{$m = 0, \ldots, M-1$}
        \STATE \COMMENT{all workers execute in parallel}
        \STATE $g^m_t \gets \nabla F(x^m_t; \xi^m_t)$ \COMMENT{stochastic gradient; SP-reduced if $P_{\text{SP}}>1$}
        \STATE $\hat{g}^m_t \gets \mathrm{clip}(g^m_t, \rho)$ \COMMENT{clip to radius $\rho$}

        \IF{$t \bmod K_u = 0$}
            \STATE $u^m_t \gets \beta_1 \mathbb{E}_m[u^m_{t-1}] + (1-\beta_1)\hat{g}^m_t$ \COMMENT{sync $u$}
        \ELSE
            \STATE $u^m_t \gets \beta_1 u^m_{t-1} + (1-\beta_1)\hat{g}^m_t$
        \ENDIF

        \IF{$t \bmod K_v = 0$}
            \STATE $v^m_t \gets \beta_2 \mathbb{E}_m[v^m_{t-1}] + (1-\beta_2)(\hat{g}^m_t \odot \hat{g}^m_t)$ \COMMENT{sync $v$}
        \ELSE
            \STATE $v^m_t \gets \beta_2 v^m_{t-1} + (1-\beta_2)(\hat{g}^m_t \odot \hat{g}^m_t)$
        \ENDIF

        \STATE $d^m_t \gets \dfrac{\eta_t}{\sqrt{v^m_t+\lambda^2}} \odot u^m_t$ \COMMENT{bias-corrected step}

        \IF{$t \bmod K_x = 0$}
            \STATE $x^m_{t+1} \gets \mathbb{E}_m[x^m_t] - d^m_t$ \COMMENT{sync $x$}
        \ELSE
...
%=============================================================================
%=============================================================================
\section{Derivation of the Momentum Drift Bounds (\texorpdfstring{\eqref{eq:drift_u}}{Eq. drift-u} and \texorpdfstring{\eqref{eq:drift_v}}{Eq. drift-v})}
...
\item {\bf Claims}
    ...
    \item[] Justification: The abstract states three quantitative claims ($303{\times}$ communication reduction over DDP, $1.78{\times}$ over DES-LOC, perplexity within $4.6\%$ of baseline) and a convergence guarantee. All are supported by Theorem~\ref{thm:sgdm} (Section~\ref{sec:convergence}) and the experimental evaluation (Section~\ref{sec:evaluation}).
    \item[] Guidelines:
    \begin{itemize}
...
\item {\bf Limitations}
    ...
    \item[] Answer: \answerYes{}
    \item[] Justification: The conclusion (Section~\ref{sec:conclusion}) identifies open directions including layer-wise synchronization, adaptive frequencies, and compressed updates. The theoretical analysis is limited to smooth non-convex objectives under bounded gradient assumptions, which are stated explicitly in the assumptions of Theorem~\ref{thm:sgdm}.
    \item[] Guidelines:
    \begin{itemize}
...
\item {\bf Theory assumptions and proofs}
    ...
    \item[] Answer: \answerYes{}
    \item[] Justification: Assumptions are stated explicitly (Section~\ref{sec:convergence}). The full proof of Theorem~\ref{thm:sgdm} is provided in Appendix~\ref{sec:sgdm_proof}, including all supporting lemmas with complete proofs in Appendix~\ref{sec:key_lemmas}. The Adam extension is given in Appendix~\ref{sec:adam_extension}.
    \item[] Guidelines:
    \begin{itemize}
    ...
    \item[] Answer: \answerYes{}
    \item[] Justification: Section~\ref{sec:experimental_setup} and Appendix~\ref{sec:appendix_a} provide model architectures, optimizer hyperparameters (including sweep results), sequence-parallel configuration, batch sizes, learning rate schedules, and random seeds. The training data (FineWeb-Edu) is publicly available.
    \item[] Guidelines:
    \begin{itemize}
...
\item {\bf Open access to data and code}
    ...
    \item[] Justification: Code is not released at submission time to preserve anonymity. The training data (FineWeb-Edu) is publicly available. We plan to release the code upon acceptance.
    \item[] Guidelines:
    \begin{itemize}
...
\item {\bf Experimental setting/details}
    ...
    \item[] Answer: \answerYes{}
    \item[] Justification: Section~\ref{sec:experimental_setup} describes the experimental setup. Appendix~\ref{sec:appendix_a} provides full architecture details, hyperparameter sweep procedures, and the final selected hyperparameters for each configuration.
    \item[] Guidelines:
    \begin{itemize}
        \item The answer \answerNA{} means that the paper does not include experiments.
...
\item {\bf Experiment statistical significance}
    ...
    \item[] Answer: \answerNo{}
    \item[] Justification: Due to the high computational cost of pretraining language models at the 135M--1.3B scale, each configuration was run once with a fixed seed. We report final perplexity values rather than confidence intervals. The deterministic seed and full hyperparameter specification enable exact replication.
    \item[] Guidelines:
    \begin{itemize}
        \item The answer \answerNA{} means that the paper does not include experiments.
...
\item {\bf Experiments compute resources}
    ...
    \item[] Answer: \answerYes{}
    \item[] Justification: Appendix~\ref{sec:appendix_a} specifies the GPU types (NVIDIA H100 SXM, B200), interconnect bandwidths (NVLink, inter-node), and the number of GPUs per experiment. Throughput measurements and wall-clock times are reported throughout Section~\ref{sec:evaluation}.
    \item[] Guidelines:
    \begin{itemize}
        \item The answer \answerNA{} means that the paper does not include experiments.
...
\item {\bf Code of ethics}
    ...
    \item[] Answer: \answerYes{}
    \item[] Justification: This work proposes a communication-efficient distributed training algorithm. It does not involve human subjects, personal data, or dual-use concerns. The research conforms with the NeurIPS Code of Ethics.
    \item[] Guidelines:
    \begin{itemize}
...
\item {\bf Broader impacts}
    ...
    \item[] Justification: FAUST is a systems-level optimization for distributed training communication. It does not introduce new model capabilities, datasets, or applications. Its primary societal effect is reducing the energy and hardware cost of large-scale training, which is broadly positive. We do not foresee negative societal impacts specific to this contribution beyond those inherent to large language model training in general.
    \item[] Guidelines:
    \begin{itemize}
...
\item {\bf Safeguards}
    ...
    \item[] Answer: \answerNA{}
    \item[] Justification: This paper does not release pre-trained models, datasets, or other artifacts that pose misuse risks. The contribution is a training algorithm and system.
    \item[] Guidelines:
    \begin{itemize}
...
\item {\bf Licenses for existing assets}
    ...
    \item[] Answer: \answerYes{}
    \item[] Justification: We cite the original papers for all models (Llama-3.1), datasets (FineWeb-Edu), and baseline methods (Local SGD, FedAvg, Local Adam, DiLoCo, DES-LOC) used in this work. FineWeb-Edu is released under an open license.
    \item[] Guidelines:
    \begin{itemize}
...
\item {\bf New assets}
    ...
    \item[] Answer: \answerNA{}
    \item[] Justification: No new datasets or pre-trained models are released in this submission.
    \item[] Guidelines:
    \begin{itemize}
...
\item {\bf Declaration of LLM usage}
    ...
    \item[] Answer: \answerYes{}
    \item[] Justification: Section~\ref{sec:llm_usage} declares that LLMs were used only for editing and improving text quality. No LLM was used for generating scientific content, proofs, or experimental results.
    \item[] Guidelines:
    \begin{itemize}
